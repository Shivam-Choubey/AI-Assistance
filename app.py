...
def speak_text(text):
    ...
    # Run speech in a separate thread to avoid blocking the main thread
    thread = threading.Thread(target=run_speech)
    thread.start()
...
def add_task_with_schedule(task, due_date, priority):
    """Adds a task to the database with a due date and priority."""
    conn = sqlite3.connect('task_manager.db')
    c = conn.cursor()
    c.execute('INSERT INTO tasks (task, due_date, priority) VALUES (?, ?, ?)', (task, due_date, priority))
    conn.commit()
...
def recognize_speech():
    """Recognizes and returns speech as text using Google Speech Recognition."""
    recognizer = sr.Recognizer()
    with sr.Microphone() as source:
        print("Listening...")
        speak_text("Listening...")
...
@app.route('/', methods=['GET', 'POST'])
def index():
    """Displays the task list and handles task-related actions."""
    if request.method == 'POST':
        if 'add_task' in request.form:
            task = request.form['task']
            due_date = request.form.get('due_date')
...
@app.route('/voice', methods=['GET', 'POST'])
def voice():
    """Handles voice commands for adding, completing, and deleting tasks."""
    if request.method == 'POST':
        if 'add_task' in request.form:
            speak_text("Please say the task")
            task = recognize_speech()
